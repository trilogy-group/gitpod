...
func (tm *tasksManager) Run(ctx context.Context, wg *sync.WaitGroup) {
	...
	for i, t := range runContext.tasks {
...
func (task *task) getCommands(context *runContext) []*string {
	...
	if context.contentSource == csapi.WorkspaceInitFromPrebuild {
		// prebuilt
		index := strconv.Itoa(len(context.tasks))
...
func (tm *tasksManager) watch(index int, task *task, terminal *terminal.Term) {
	...
	go func() {
		fileName := "/workspace/.gitpod/prebuild-log-" + strconv.Itoa(index)
